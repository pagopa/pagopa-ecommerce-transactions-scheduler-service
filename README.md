...
| PENDING_TRANSACTIONS_SEND_PAYMENT_RESULT_TIMEOUT_SECONDS     | Timeout for a send payment result to be received. This timeout is evaluated for a transaction stuck in CLOSED status for which an OK response has been received by Nodo avoiding set as expired those transaction for which a send payment result can be still received      | int     |         |

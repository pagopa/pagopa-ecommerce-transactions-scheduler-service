...
| TRANSACTIONVIEW_UPDATE_ENABLED                               | Flag to enable/disable transaction view collection updates in MongoDB. When true (default), the publisher updates the transaction-view collection. When false, only the event store is updated, skipping the view update.                                                                           | boolean | true    |

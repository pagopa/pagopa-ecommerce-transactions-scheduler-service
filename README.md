...
| MIGRATION_TRANSACTION_BATCH_EVENTSTORE_LOCK_TTL_SECONDS           | TTL (Time To Live) in seconds for the distributed lock used by the eventstore migration batch to ensure exclusive execution across multiple service instances                                                                                                                                       | int     |         |
...
| MIGRATION_TRANSACTION_BATCH_TRANSACTIONS_VIEW_LOCK_TTL_SECONDS    | TTL (Time To Live) in seconds for the distributed lock used by the transactions view migration batch to ensure exclusive execution across multiple service instances                                                                                                                                | int     |         |

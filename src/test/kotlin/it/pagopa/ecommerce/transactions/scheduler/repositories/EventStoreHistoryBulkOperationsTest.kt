...
class EventStoreHistoryBulkOperationsTest {

    @Mock lateinit var reactiveMongoTemplate: ReactiveMongoTemplate

    @Mock lateinit var bulkOps: ReactiveBulkOperations

    private lateinit var service: EventStoreHistoryBulkOperations

    @BeforeEach
    fun setUp() {
        service = EventStoreHistoryBulkOperations(reactiveMongoTemplate)
    }
    ...
    fun `bulkUpsert should handle empty input flux`() {
        ...
        Mockito.verify(reactiveMongoTemplate, Mockito.never())
            .bulkOps(any(BulkOperations.BulkMode::class.java), eq(BaseTransactionEvent::class.java))
    }
}
